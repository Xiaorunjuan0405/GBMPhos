...
class MultiScaleConvNet(nn.Module):
    def __init__(self):
        ...
        # 第一层卷积，对原始序列进行卷积
        self.conv1 = nn.Conv1d(in_channels=55, out_channels=128, kernel_size=1, padding=1)

        # 第二层卷积，对不同尺度下的序列进行卷积
        self.conv2_1 = nn.Conv1d(in_channels=128, out_channels=32, kernel_size=3, padding=1)
        self.conv2_2 = nn.Conv1d(in_channels=128, out_channels=32, kernel_size=5, padding=2)
        self.conv2_3 = nn.Conv1d(in_channels=128, out_channels=32, kernel_size=7, padding=3)


        # 池化层，将不同尺度下的特征进行池化
        self.pool = nn.MaxPool1d(kernel_size=2, stride=1)

        self.gru = nn.LSTM(input_size=128, hidden_size=32, bidirectional=True, batch_first=True)
        ...
        self.bn3 = nn.BatchNorm1d(2176)

        # 全连接层，将不同尺度下的特征拼接在一起并进行分类
        self.fc = nn.Linear(in_features=2176, out_features=512)
    ...
    def forward(self, x):
        ...
        out1 = self.conv1(x)
        out1 = self.pool(out1)
        conv_out1 = self.bn1(out1)  #128,64,34
...
if __name__ == '__main__':
    ...
    x_train, x_test, y_train, y_test = train_test_split(data, label, test_size=0.2, random_state=88,stratify=label)
    ...
    best_acc = 0  # Variable to store the highest accuracy
    best_epoch = 0  # Variable to store the epoch with the highest accuracy
    best_sn = 0  # Variable to store the highest sensitivity
    best_sp = 0  # Variable to store the highest specificity
    best_mcc = 0  # Variable to store the highest MCC
    best_auc = 0
