...
class MultiScaleConvNet(nn.Module):
    def __init__(self):
        ...
        self.conv2_2 = nn.Conv1d(in_channels=55, out_channels=32, kernel_size=3, padding=1)
        self.conv2_3 = nn.Conv1d(in_channels=55, out_channels=32, kernel_size=3, padding=1)


        # 池化层，将不同尺度下的特征进行池化
        self.pool = nn.MaxPool1d(kernel_size=2, stride=1)

        self.gru = nn.GRU(input_size=55, hidden_size=32, bidirectional=True, batch_first=True)
