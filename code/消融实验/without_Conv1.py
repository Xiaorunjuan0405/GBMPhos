...
class MultiScaleConvNet(nn.Module):
    def __init__(self):
        ...
        # 第二层卷积，对不同尺度下的序列进行卷积
        self.conv2_1 = nn.Conv1d(in_channels=55, out_channels=32, kernel_size=3, padding=1)
        self.conv2_2 = nn.Conv1d(in_channels=55, out_channels=32, kernel_size=5, padding=2)
        self.conv2_3 = nn.Conv1d(in_channels=55, out_channels=32, kernel_size=7, padding=3)


        # 池化层，将不同尺度下的特征进行池化
        self.pool = nn.MaxPool1d(kernel_size=2, stride=1)

        self.gru = nn.LSTM(input_size=55, hidden_size=32, bidirectional=True, batch_first=True)
        ...
        self.bn3 = nn.BatchNorm1d(3136)

        # 全连接层，将不同尺度下的特征拼接在一起并进行分类
        self.fc = nn.Linear(in_features=3136, out_features=512)
    ...
    def forward(self, x):
        ...
        out1 = x.transpose(dim0=1, dim1=2)
