...
class MultiScaleConvNet(nn.Module):
    def __init__(self):
        ...
        # 第二层卷积，对不同尺度下的序列进行卷积
        self.conv2_1 = nn.Conv1d(in_channels=128, out_channels=32, kernel_size=1, padding=1)
        self.conv2_2 = nn.Conv1d(in_channels=128, out_channels=32, kernel_size=1, padding=1)
        self.conv2_3 = nn.Conv1d(in_channels=128, out_channels=32, kernel_size=1, padding=1)
        ...
        self.bn3 = nn.BatchNorm1d(3296)

        # 全连接层，将不同尺度下的特征拼接在一起并进行分类
        self.fc = nn.Linear(in_features=3296, out_features=512)
