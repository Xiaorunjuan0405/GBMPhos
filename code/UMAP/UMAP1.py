...
class MultiScaleConvNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # 将不同尺度下的特征进行分类
        out1 = self.fc(torch.cat([out1, att_output], dim=1))
        out = self.fc1(out1)
        # out = self.drop(out)
        out = self.fc2(out)
        # out = self.drop(out)
...
if __name__ == '__main__':
    ...
    for epoch in range(60):
        ...
        if acc > 0.85 and acc > best_acc:
            # torch.save(model.state_dict(),'../../best_model/model_weights.pt')
            torch.save(model, str(epoch) + str(acc) + '.pth')
            best_acc = acc
