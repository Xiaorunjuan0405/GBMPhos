...
class MultiScaleConvNet(nn.Module):
    def __init__(self):
        ...
        # 第二层卷积，对不同尺度下的序列进行卷积
        self.conv2_1 = nn.Conv1d(in_channels=128, out_channels=64, kernel_size=3, padding=1)
        self.conv2_2 = nn.Conv1d(in_channels=128, out_channels=64, kernel_size=3, padding=1)
        self.conv2_3 = nn.Conv1d(in_channels=128, out_channels=64, kernel_size=3, padding=1)
        ...
        self.fc = nn.Linear(in_features=4288, out_features=512)
        self.fc1 = nn.Linear(in_features=512, out_features=16)
...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=0.001, weight_decay=5e-04)
