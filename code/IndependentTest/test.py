...
class MultiScaleConvNet(nn.Module):
    def __init__(self):
        ...
        self.conv2_2 = nn.Conv1d(in_channels=64, out_channels=32, kernel_size=3, padding=2)
        self.conv2_3 = nn.Conv1d(in_channels=64, out_channels=32, kernel_size=3, padding=3)

        # 池化层，将不同尺度下的特征进行池化
        self.pool = nn.MaxPool1d(kernel_size=2, stride=1)

        self.gru = nn.LSTM(input_size=64, hidden_size=32, bidirectional=True, batch_first=True)
...
if __name__ == '__main__':
    ...
    x_train, x_test, y_train, y_test = train_test_split(all_data, label, test_size=0.2, random_state=100)
    ...
    model = torch.load('model_epoch_48.pth').to(device)
