...
class MultiScaleConvNet1(nn.Module):
    def __init__(self):
        super(MultiScaleConvNet1, self).__init__()
        ...
        self.lstm = nn.LSTM(input_size=64, hidden_size=32, bidirectional=True, batch_first=True)
...
def test(model, test_loader, device, label):
    ...
    # Plot ROC curve
    plt.plot(fpr, tpr, label='{} (AUC = {:.4f})'.format(label, roc_auc_value))

    return sn, sp, mcc, acc, roc_auc_value
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('../../best_model1/BiGRU_model_weights.pt'))
    model_bilstm = MultiScaleConvNet1().to(device)
    model_bilstm.load_state_dict(torch.load('../../best_model1/BiLSTM_model_weights.pt'))

    # Existing ROC curve plot code
    plt.figure()
    ...
    # Call the test function for each additional model
    test(model, test_loader, device, 'BiGRU')
    test(model_bilstm, test_loader, device, 'BiLSTM')
    # Add more test calls for additional models as needed
