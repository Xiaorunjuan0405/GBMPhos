...
def test(model, test_loader, device):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            output,att_weight = model(data,512)
            output = output.squeeze()
...
if __name__ == '__main__':
    ...
    for epoch in range(100):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            target = target.float().to(device)
            # 前向传播
            output, attn_weights = model(data,512)
            # 计算损失
            # L1正则化
        ...
        #     plt.imshow(attn_matrix, cmap='Reds', vmin=attn_matrix.min(), vmax=attn_matrix.max(), origin='lower')
        ...
        # plt.imshow(mean_attn_weights, cmap='Reds', vmin=mean_attn_weights.min(), vmax=mean_attn_weights.max(),
        #            origin='lower')
