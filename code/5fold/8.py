...
class MultiScaleConvNet(nn.Module):
    def __init__(self):
        ...
        # 第一层卷积，对原始序列进行卷积
        self.conv1 = nn.Conv1d(in_channels=55, out_channels=128, kernel_size=1)

        # 第二层卷积，对不同尺度下的序列进行卷积
        self.conv2_1 = nn.Conv1d(in_channels=128, out_channels=64, kernel_size=3)
        self.conv2_2 = nn.Conv1d(in_channels=128, out_channels=64, kernel_size=3)
        self.conv2_3 = nn.Conv1d(in_channels=128, out_channels=64, kernel_size=3)

        # 池化层，将不同尺度下的特征进行池化
        self.pool = nn.MaxPool1d(kernel_size=2, stride=1)

        self.gru = nn.LSTM(input_size=128, hidden_size=64, bidirectional=True, batch_first=True)
        ...
        self.fc = nn.Linear(in_features=5952, out_features=1024)
