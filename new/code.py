...
if __name__ == '__main__':
    ...
    seed_torch()
    all_data, seq, label = load_data('ST_1.csv')
    onehot_out = onehot_encoding(seq)

    blo_out, label = load_and_encoding("BLOSUM62/data1.csv", 20)
    zscale_out, label = load_and_encoding('Zscale/data1.csv', 5)
    bin51_out, label = load_and_encoding('Binary51/data1.csv', 5)
    bin52_out,label = load_and_encoding('Binary52/data1.csv', 5)
    ...
    data = torch.cat((blo_out,onehot_out,zscale_out, bin51_out,bin52_out), dim=2)
    ...
    model = torch.load('../best_model/620.8461538461538461.pth').to(device)
